...
/** Run ffmpeg one-shot HLS + thumbnail  */
async function transcodeToHls(inputFile: string, outDir: string) {
  ...
  await runCommand(
    'ffmpeg',
    ['-y', '-ss', '00:00:03', '-i', inputFile, '-frames:v', '1', '-q:v', '2', thumbFile],
    outDir
  );
...
async function handleJob(jobData: TranscodeJobData) {
  ...
  logger.info({ videoId, srcObjectKey, tmpBase }, `Starting job for video`);
  ...
  try {
    ...
    try {
      ...
      logger.warn({ videoId, err: e }, "Couldn't set video processing status (maybe id mismatch)");
    }

    // download source
    logger.info({ videoId, srcObjectKey }, 'Downloading source from MinIO...');
    await fGetObjectToFile(MINIO_BUCKET, srcObjectKey, inputFile);

    logger.info({ videoId, inputFile, outDir }, 'Running ffmpeg to produce HLS renditions...');
    const { outDir: producedDir, thumbFile } = await transcodeToHls(inputFile, outDir);

    // upload everything under hls/{videoId}/
    const remotePrefix = path.posix.join('hls', videoId);
    logger.info({ videoId, remotePrefix }, 'Uploading HLS output to MinIO');
      ...
      data: [
        {
          ...
          bitrateKbps: 5000,
          codec: 'h264',
          playlistUri: `${remotePrefix}/0/index.m3u8`,
        },
        {
          videoId,
          height: 720,
          bitrateKbps: 2800,
          codec: 'h264',
          playlistUri: `${remotePrefix}/1/index.m3u8`,
        },
        {
          videoId,
          height: 480,
          bitrateKbps: 1400,
          codec: 'h264',
          playlistUri: `${remotePrefix}/2/index.m3u8`,
        },
        {
          videoId,
          height: 144,
          bitrateKbps: 200,
          codec: 'h264',
          playlistUri: `${remotePrefix}/3/index.m3u8`,
        },
      ],
    });
    ...
    try {
      await prisma.$transaction([
        ...
        jobId
          ? prisma.job.update({ where: { id: String(jobId) }, data: { status: 'completed' } })
          : prisma.$queryRaw`select 1`,
      ]);
    } catch (e: unknown) {
      logger.warn({ videoId, err: e }, 'DB update issue');
    }

    logger.info({ videoId }, 'Transcode completed');
  } catch (err: unknown) {
    logger.error({ videoId, err }, 'Job failed');
    try {
      await prisma.video.update({ where: { id: videoId }, data: { status: 'failed' } });
      if (jobId)
        await prisma.job.update({ where: { id: String(jobId) }, data: { status: 'failed' } });
    } catch (e: unknown) {
      logger.warn({ videoId, err: e }, 'Failed to mark DB entities as failed');
    }
    throw err;
  } finally {
    // cleanup
    try {
      ...
      logger.warn({ videoId, err: e }, 'cleanup failed');
    }
  }
}
...
worker.on('failed', async (job, err) => {
  ...
  // If job has exhausted all retry attempts, move it to DLQ
  if (job && job.attemptsMade === job.opts.attempts) {
    try {
      ...
      logger.info({ jobId: job.id }, 'Job moved to Dead Letter Queue after exhausting retries.');
    } catch (dlqErr) {
      logger.error({ jobId: job.id, err: dlqErr }, 'Failed to add job to Dead Letter Queue');
    }
  }
});
